...
public class GroupProMethodMain
{
	public static void main(String[] args)
	{
		...
		// 배열 선언
		// 지출금 내역 저장 배열
		int[][] arrPaymentDetail = new int[daysCount][MAX_DATA];
		// 지출 사유 저장 배열
		int[][] arrPayReason = new int[daysCount][MAX_DATA];
		...
		{
			System.out.println((count+ 1) + "번 째 날의 데이터를 입력합니다. 최대 10개 지출액 0 입력시 입력종료");
			for (int i = 0; i < MAX_DATA; i++)
			{// 지출항목 입력
				// 지출항목 변수 선언
				int paymentOneCount = 0;
				// n번째 일자에 n번째 지출액 저장
				System.out.printf("%d번째 데이터입니다.\n 지출액을 입력해주세요 : ", count + 1);
				paymentOneCount = gpc.inputInt(sc.nextLine());
				arrPaymentDetail[count][i] = paymentOneCount;
				
				if(paymentOneCount <= 0)
				{
					break;
				}
				// 지출항목 입력
				System.out.printf("%d번째 데이터입니다.\n 지출 항목을 입력해주세요 : ", count + 1);
				String payReason = sc.nextLine();

				// n번째 일자에 지출 사유 저장
				arrPayReason[count][i] = gpc.convertReasonToInt(payReason);
			}
			count++;
		}
...
//			- 조건식 : i < daysCount
//			- 증감식 : i++
		categoryCountSum = gpc.getCategoryCountArray(arrPaymentDetail, arrPayReason, false);
		categoryPaymentSum = gpc.getCategoryCountArray(arrPaymentDetail, arrPayReason, true);
		

		for (int i = 0; i < categoryPaymentSum.length; i++)
		{ // 항목별 지출 합계가 담긴 배열의 크기만큼 반복한다.
...
//				1. 내용출력
			...
			if(optionSelect == 4)
			{
				...
				// 일자별 금액 합을 저장할 배열 선언
				int[] arrDailyPaymentSum = new int[daysCount];
				for (int i = 0; i < arrPaymentDetail.length; i++)
				{
					for (int j = 0; j < MAX_DATA; j++)
					{
						// 일자별 금액이 0이 입력된 데이터가 있으면 데이터를 더이상 입력하지 않는다.
						if(arrPaymentDetail[i][j] == 0)
						{
							break;
						}
						
						arrDailyPaymentSum[i] += arrPaymentDetail[i][j];
					}
				}
				// for문으로 현재 지출과 이전 지출을 비교
				// 2) for문 (초기식 int i =1, 조건식 arrPaymentDetail.length (지출금내역 저장 배열의 길이),증감식++)
				for (int i = 1; i < arrDailyPaymentSum.length; i++)
				{
					
					// 2-1) if(arrPaymentDetail[i] > arrPaymentDetail[i-1]) 이라면 / 지금 확인하는 지출액이 이전
					// 지출액보다 크다면
					if(arrDailyPaymentSum[i] > arrDailyPaymentSum[i - 1])
					{
						...
						{
							increasedAmounts += "\n" + arrDailyPaymentSum[i - 1] + " - " + arrDailyPaymentSum[i];
						}
						else
						{
							// +) 아닐땐 문자열에 " - " + 지금 확인하는 지출액을 문자열에 더해 저장해줌
							increasedAmounts += " - " + arrDailyPaymentSum[i];
						}
