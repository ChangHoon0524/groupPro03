 ...
 * 그룹프로젝트 Method화 
 * */
public class GroupPro03_Method
{
	// 전역상수로 변경
	// 지출항목 상수 선언
	// 기타 : 0, 식비 : 1, 교통 : 2, 쇼핑 : 3
	...
	{
		GroupPro03_Method gr_mt = new GroupPro03_Method();
		// 20260112 SCH 작성
		Scanner sc = new Scanner(System.in);
		// 입력 관련 처리
		// 일자 입력
		System.out.print("지출을 기록할 일일을 입력해주세요.(최소 1일) : ");
		int daysCount = parseIntInput(sc);
		// 예산 입력
		System.out.print("예산을 입력해주세요 : ");
		int budget = parseIntInput(sc);
		...
		// inputData 메서드를 통해 값을 입력받는다.
		gr_mt.inputData(sc, daysCount, arrPaymentDetail, arrPayReason);
...
//							2.categoryPaymentSum[0] +arrPaymentDetail[i]
		...
		while (true)
		{
			System.out.print("기능을 선택 하세요." + "\n 1. 총 지출 금액 출력" 
					...
					+ "\n 4. 지출 금액 연속 증가 여부 확인" 
					+ "\n 5. 예산검정" + "\n 0. 시스템 종료 :");
			int optionSelect = parseIntInput(sc);
...
//		                System.out.println();//// 의미없음 가독성용
	...
	// Integer.ParseInt(nextLine())을 자주사용하므로 메서드화
	// 다른 메서드에서도 사용할 수 있으므로 static화 및 int 입력시의 발생 오류 예외처리 추가
	static int parseIntInput(Scanner scanner)
	{
		int parseValue;
		try
		{
			 parseValue = Integer.parseInt(scanner.nextLine()); 
		}
		catch(NumberFormatException ex)
		{
			System.out.println("잘못된 숫자 입력으로 0이 입력되었습니다.");
			parseValue = 0; 
		}
		return parseValue;
	}
	...
	// 배열에 데이터를 넣어줄 메서드
	void inputData(Scanner scanner, int daysCount, int[] arrPayment, int[] arrPayReason) {
		int count = 0;
		while(count < daysCount)
		{
			// 지출항목 입력
			System.out.printf("현재일자 : %d, 지출 항목을 입력해주세요 : ", count + 1);
			String payReason = scanner.nextLine();
			// 지출항목 변수 선언
			int convertReason = 0;
...
//			arrPayment[count] = Integer.parseInt(scanner.nextLine());
			arrPayment[count] = GroupPro03_Method.parseIntInput(scanner);
